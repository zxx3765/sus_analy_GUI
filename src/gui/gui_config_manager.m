...
%   handles - 更新后的句柄结构体
    ...
    configPanel = uipanel('Parent', parent, ...
                         'Title', '⚙️ 分析配置', ...
                         'Position', [0.02, 0.02, 0.96, 0.715], ...
                         'FontSize', 10, ...
                         'FontWeight', 'bold', ...
                         'ForegroundColor', [0.2, 0.6, 0.2]);
    
    %% 基本配置区域
    % 从面板实际可见区域内开始排布，避免高 DPI/标题边框下顶部控件重叠。
    y_pos = 485;
    ...
    % 新增：频带RMS对比分析
    y_pos = y_pos - 25;
    handles.bandRmsAnalysisCheck = uicontrol('Parent', configPanel, ...
                                            'Style', 'checkbox', ...
                                            'String', '频带RMS对比分析', ...
...
function updateConfig(src, ~, handles)
    ...
    try
        ...
        if isfield(handles, 'bandRmsAnalysisCheck')
            config.analysis.band_rms = logical(get(handles.bandRmsAnalysisCheck, 'Value'));
        end

        % 保留已加载或脚本设置的频带参数，仅由复选框控制开关。
        if isfield(handles, 'config') && isfield(handles.config, 'band_rms')
            config.band_rms = handles.config.band_rms;
        end
